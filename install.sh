...
if [[ "$CLAUDE_OK" == "0" ]]; then
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]] && [[ -n "$npm_prefix" ]] && [[ ! -w "$npm_prefix/lib/node_modules" ]]; then
        if sudo -n true 2>/dev/null; then
            ...
            step_fail "需要 root 权限，请手动执行: sudo npm install -g @anthropic-ai/claude-code@2.1.233"
            exit 1
        fi
    fi
    ...
    if npm_direct $npm_cmd install -g --no-audit --no-fund @anthropic-ai/claude-code@2.1.233 --registry="$NPM_REGISTRY"; then
        CLAUDE_VER=$(claude --version 2>/dev/null) || true
        step_ok "Claude Code 安装成功"
        install_ok=1
    else
        step_fail "安装失败，请手动执行: $npm_cmd install -g @anthropic-ai/claude-code@2.1.233"
    fi
